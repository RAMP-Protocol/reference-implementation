...
(emit-unpopulated ``protojson`` — camelCase fields, unset optional fields
omitted):

* the signed delivery URL rides on the top-level ``retrievalEndpoint`` field
  (canonical ``retrieval_endpoint``; the legacy top-level ``signed_url`` slot is
  gone). ADR-019: ``licensed`` is derived from its PRESENCE.
* the refusal cause rides on the typed ``absenceReason`` field
  (``OfferAbsenceReason``).
* the delivered price rides on the typed ``cost`` field (canonical ``Cost`` —
  ``amount`` = unit_cost × est_quantity, plus ``unitCost`` and ``currency``).
...
def absence_reason_of(payload: object) -> str | None:
    """Return the typed refusal cause (``absenceReason`` OfferAbsenceReason), or None.

    Reads the protojson camelCase ``absenceReason`` with a snake-case
    ``absence_reason`` fallback. None when licensed or for a non-dict payload.
    """
...
def retrieval_endpoint_of(payload: object) -> str | None:
    """Return the canonical signed URL (top-level ``retrievalEndpoint``), or None.

    Reads the protojson camelCase ``retrievalEndpoint`` with a snake-case
    ``retrieval_endpoint`` fallback for any UseProtoNames config. Returns None for
    a non-dict payload or an absent/empty field — never raises.
    """
    if not isinstance(payload, dict):
        return None
...
def first_item_of(payload: object) -> dict[str, Any] | None:
    ...
    offer is a 1-element batch), so the per-result fields (``retrievalEndpoint``,
    ``transactionId``, ``cost``, ``denialReason``) live in ``items[0]`` — not at
...
def offer_exchanges_by_uri(payload: object) -> dict[str, set[str]]:
    ...
    returns one ``OfferGroup`` per requested URI (canonical proto-JSON camelCase
    ``offerGroups``/``uri``/``offers``/``exchange``), each group's offers stamped
...
def absence_reasons_by_uri(payload: object) -> dict[str, str]:
    """Map each ``OfferGroup.uri`` to its per-group ``absenceReason``, when set.

    A URI miss across every authorized exchange surfaces as a typed-absence
    group: ``offers`` empty and ``absenceReason`` carrying the ADR-008 D2
...
def cost_of(payload: object) -> dict[str, Any] | None:
    ...
    Reads the protojson camelCase ``cost`` with a snake-case fallback. The Cost
    message carries ``amount`` (unit_cost × est_quantity), ``unitCost`` (the
