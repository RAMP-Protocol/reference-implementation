...
- **Repository**: `ramp.agents.agent_id` is the PK with no `tenant_id` column. Per-tenant tables (`transaction_log`, `reporting_obligations`, `catalog`) reference it directly. CLAUDE.md Rule 4 (tenant isolation) applies to those per-tenant tables, not to `agents` itself — Rule 4 governs list/scan queries that need tenant scoping, and a PK lookup of a global identity is not a violation (framing landed in the MR !1 follow-up comment).
...
**Overlap window.** The protocol's rotation model (proto §1693-1696, ADR-003 §3) is "publish the new kid alongside the old, switch signing to the new, remove the old after the grace window." The Exchange treats `WellKnownManifest.public_keys` as a set: any key whose `[not_before, not_after)` covers `Clock.Now()` is accepted (per ADR-008 D1, the clock is a port). The 5-minute TTL bounds the lag between "operator removed the kid from the URL" and "last Exchange still accepts a signature under it"; ADR-003 §3b walks the urgent-compromise variant.
...
**Code site.** `src/exchange/internal/service/marketplace.go` replaces the per-transaction `sha256` with a `jwk.ThumbprintEd25519` helper following RFC 7638 §3.2. The same helper is reusable by the Edge (TS) for `thumbprint(presented_key)`, by the ADR-011 reconciler, and by the Exchange's signing path for the `agent_id` URL parameter.
