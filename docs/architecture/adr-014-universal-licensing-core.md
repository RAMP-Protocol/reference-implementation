...
**Invariants.** The first three are hard rejects, each with a row in *Validation rules (normative)* below. The fourth is not a reject at all: it binds the consumer that reads the term, not the push that stores it, which is why it has no row there.
...
- `permitted ∩ prohibited` non-empty: rejected. Checked twice, over two readings of the same two lists — the tokens as received, and the tokens the canonicalisation fold produces, since one token can be written more than one way.
...
The registered tokens are proto-native: the `(ramp.v1.vocab_enum)` entries on the `RestrictionKind` enum values in `ramp.proto` (module `github.com/RAMP-Protocol/protocol`) are the sole authored source. The token list is authored once, on the enum value. Accepted aliases are authored the same way, beside the tokens they resolve to, as `(ramp.v1.vocab_enum_alias) = "alias=token"` entries on the same enum value. The generated per-axis packages carry both: `IsRegistered` for membership, and `Aliases` (alias → registered token) with `Canonical` (an alias becomes its registered token; anything else is returned unchanged) for normalization. The alias notes in the tables below restate those authored entries, and normalization cannot drift from the registry.
...
**`USER_TYPE`** — `individual`, `academic`, `non_profit`, `news_publisher`, `broadcaster`, `commercial_entity`. Aliases: `personal` → `individual`; `business` → `commercial_entity`; `enterprise` → `commercial_entity`. Custom values namespaced.
...
**Consequences.**
- The projection — `selectTerms` in `src/exchange/internal/service/termselect.go` — is **scope-only** (the single projection shared by discovery-signing and execute-verification, so offer signatures stay byte-identical). The requester-axis machinery (`requesterAxes`, `restrictionsSatisfied`, the `intended_use`→FUNCTION wiring) is deleted.
...
- **Stored-side restriction handling is untouched:** normalization (`helpers.NormalizeResourceEntry` — fold, then `Canonical` alias resolution) and the ingest-tier checks (`helpers.ValidateLicenseTerm` — registry membership) still run at ingest, from the SDK's `sdk/go/helpers` package in the module `github.com/RAMP-Protocol/protocol`, so restrictions on returned offers are canonical. An unregistered `Restriction` token is a **lint warning** (term accepted), decoupled from the `advisory` flag, never a hard-reject — under scope-only projection it can never gate access. (Pricing/Quota metering-token membership remains a hard-reject; that is unrelated to restriction membership.)
...
**Registry:** proto-native. The registered tokens are authored as buf custom options directly on the fields / enum values that use them — `(ramp.v1.vocab)` on `Quota.metric` and `Pricing.unit`, `(ramp.v1.vocab_enum)` on the `RestrictionKind` enum values, with the accepted aliases beside them as `(ramp.v1.vocab_enum_alias)` — and nowhere else. The `protoc-gen-rampvocab` buf plugin reads these options structurally and emits a typed-constant package per axis (`functiontokens`, `geographytokens`, `usertypes`, `quotametrics`, `pricingunits`) with `IsRegistered`, plus `Aliases` and `Canonical` on the enum axes; constants, membership check, alias resolution and validation all derive from the single authored list and cannot drift. Function values are seeded from RSL 1.0, geography from ISO-3166-1 (only the non-ISO specials `*`/`EU`/`EEA` are registered; alpha-2 codes are structural). Additions are pull requests that edit the option list — an additive version bump. The accepted aliases are listed beside their tokens in the Vocabulary tables above (seven on `FUNCTION`, three on `USER_TYPE`).

**Build-time check (`make quality`, hard FAIL):** the ingest package's demo-feed test replays the committed demo feeds through the same normalize/validate helpers and requires zero rejects and zero warnings. An unregistered bare token in our own fixtures is a typo to fix or a registry PR to file.

**Ingestion-time checks:** run after Gate 1 (caller signature) and Gate 2 (contributor authorization), per entry, from the SDK helpers — `helpers.NormalizeResourceEntry`, then `helpers.ValidateLicenseTerm` per term. Registry membership, described here, is not all that face checks: it also refuses a restriction whose permitted and prohibited lists name one token once folded, and warns on an `OBLIGATION_KIND_OTHER` obligation with no detail. Neither is a membership question; both are in *Validation rules (normative)* and *Lint warnings* below. Membership is enforced at two strengths, split by what the token decides. A bare `Pricing.unit` or `Quota.metric` that is not registered is a **hard reject**: the metering basis is what an agent is charged or capped by, and no transactable Offer can be made on an unknown one. A bare `Restriction` token that is not registered is a **warning** naming the token (term accepted): under scope-only projection a restriction can never gate access. Warnings are returned in `PushResourcesResponse.warnings[]`. A publisher pushing `metric: "imprssions"` therefore has the whole submission refused, with the entry and the token named; a publisher can run the same checks before pushing — `ValidateResourceEntry` in the SDK (Go `sdk/go/helpers`, TypeScript `licenseterm`, Python `ramp_sdk.licenseterm`) or `ramp-ingest --check` — and gets the verdict the Exchange will reach.

**Namespacing:** bare token = canonical registry (`ai-input`); `vendor:token` = deliberate custom (`acme:proprietary-use`). Membership is checked on bare tokens only — a reject or a warning per the split above; namespaced tokens pass silently.
...
The shape/structure rejects below are enforced by **`protovalidate` (proto CEL) at the RPC boundary**, per-term — not hand-rolled in the Exchange. The registry-membership and post-fold disjointness rejects are the **ingest tier**: `helpers.ValidateLicenseTerm` from the SDK (`sdk/go/helpers` in the module `github.com/RAMP-Protocol/protocol`), which the Exchange runs per term after `helpers.NormalizeResourceEntry` and holds no private copy of — so a publisher running the SDK's `ValidateResourceEntry` client-side reaches the verdict the Exchange will. `PushResources` is **all-or-nothing per submission**: if *any* term in *any* entry fails a hard reject, the whole submission is rejected with `InvalidArgument` (the error names the offending entry + rule) and **nothing from that submission is persisted**. There is no partial acceptance within a submission — the publisher fixes it and resubmits it whole. A feed larger than the wire bound on one request is sent as several submissions in feed order, and the ones stored before a refusal stay stored; re-sending converges, because a push upserts on the resource URI. (REFERENCE_ONLY no longer forbids machine fields — they are an advisory summary; see the Semantics section.) Disjointness is the one rule evaluated at **both** tiers, and it must be. The boundary rule compares the tokens exactly as received; several tokens have more than one accepted spelling — a registered alias beside its canonical form, and either in any ASCII case — so two spellings of one token clear the boundary and become one token when the ingest tier folds them. The ingest tier therefore re-asserts the property over the canonicalised tokens as `restriction.canonical_disjoint`. Neither rule suppresses the other, but how many refusals a caller sees depends on the face that ran. The Exchange answers one — the boundary refuses the request before the ingest tier runs — while the SDK's `ValidateResourceEntry`, which a publisher runs before pushing, composes both tiers and reports both. A deployment that does not mount the wire tier still gets the second of the two readings. Both rules are scoped to a single `Restriction`; what makes one restriction the whole of an axis is the one-restriction-per-kind rule, which stays a boundary rule, so the per-axis property is held by the two together.
...
| Bare (non-namespaced) `Pricing.unit` or `Quota.metric` token not in the registry (ingest tier) | reject |
...
| `permitted ∩ prohibited` non-empty, tokens as received (wire tier, `restriction.permitted_prohibited_disjoint`) | reject |
| `permitted ∩ prohibited` non-empty **after canonicalisation** (ingest tier, `restriction.canonical_disjoint`) | reject |
...
| Condition | Warning |
|---|---|
| Unregistered bare `Restriction` token, any axis (an unregistered bare `Pricing.unit` / `Quota.metric` is a hard reject, above) | unrecognized token — register or namespace |
...
- **2026-09-02 — Disjointness is evaluated at both tiers, over two readings of the same lists.** `restriction.permitted_prohibited_disjoint` runs at the wire tier and compares token spellings; the fold then resolves aliases and folds ASCII case, so a term naming one spelling under `permitted` and another under `prohibited` passed the boundary and was stored with the same token in both lists. Because the stored term rides on offers and the Exchange validates its own responses, every discovery request returning that resource failed with an internal error, while the push had been answered as accepted. The ingest tier now re-asserts the property over the canonical tokens as `restriction.canonical_disjoint` in `helpers.ValidateLicenseTerm`, which the Exchange already runs per term after `helpers.NormalizeResourceEntry`; no Exchange code changes. The rule has its own id because one name for two rules is what the ingest-tier id guard forbids, and it canonicalises what it compares rather than assuming a normalised input. Both disjointness rules remain scoped to a single `Restriction`; the per-axis reading is held jointly with one-restriction-per-kind, which is a wire-tier rule — this Exchange always mounts that tier, on every surface (each raw mount builds its handler options through one shared constructor that installs the validate interceptor unconditionally, and the ExchangeService mount composes the same engine in strict mode), so the per-axis property does hold here. Whether the ingest tier should aggregate per kind, and so hold that property alone, is left open. *Before:* the order was boundary, then fold, then ingest checks, and nothing re-read the tokens the fold produced.
- **2026-08-28 — Term normalization and the ingest-tier checks are the SDK's; aliases are authored in the proto.** The Exchange normalizes and validates license terms through `helpers.NormalizeResourceEntry` / `helpers.ValidateLicenseTerm` from `sdk/go/helpers` in the module `github.com/RAMP-Protocol/protocol`; its private copy of those rules is deleted, and the scope-only projection stays downstream as `selectTerms` in `src/exchange/internal/service/termselect.go`. Accepted aliases are authored beside their tokens as `(ramp.v1.vocab_enum_alias)` and generated as `Aliases`/`Canonical` (recorded in *Vocabulary*: seven on `FUNCTION`, three on `USER_TYPE`). The validation tables now agree with each other and with the implementation: an unregistered bare `Pricing.unit` / `Quota.metric` is a hard reject (ingest tier), and only an unregistered bare `Restriction` token is a warning. A publisher can reach the Exchange's verdict before pushing with the SDK's `ValidateResourceEntry` or `ramp-ingest --check`. *Before:* the Exchange held its own normalize/validate package; the lint table listed unregistered metering tokens as warnings while the hard-reject prose and both implementations rejected them; the alias set was written only in that package.
