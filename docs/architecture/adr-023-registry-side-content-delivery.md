...
**Superseded: the signer is now the SDK's, in all three languages.** The Go signing face was built app-side because the SDK had none — Python already shipped `sign_agent_binding`, TypeScript shipped the verifier the edge runs, and Go's only SDK artefact was an unexported test helper. That gap has since been closed upstream, and the registry signs through the SDK. What stays app-side is the verifying half's signature-base builder, and only that: the SDK does not export it, because only a verifier needs it. The header names come from the SDK like the signer does — one wire header, one declaration, so a rename upstream cannot leave a verifier checking a value production no longer sends. The reasoning below is kept because it still explains WHY the profile builds its own base rather than routing through the general signer.
...
Upstreaming the signer into the SDK's Go helpers, so all three languages share one implementation, was named here as desirable follow-up. It has since landed, and the registry uses it.
...
- **Redirects refused.** The proof covers `@target-uri`, so replaying it at a new location fails the edge's own check, and re-signing per hop would hand a fresh proof of possession of the agent's key to whatever host the first hop named. When this was written the SDK's own agent fetch path followed redirects, so a publisher edge that 302s worked there and was refused here; the SDK has since adopted the same refusal, for the same reason, and the two paths agree. Redirect support would need per-hop re-signing plus host anchoring, and is deferred on both.
