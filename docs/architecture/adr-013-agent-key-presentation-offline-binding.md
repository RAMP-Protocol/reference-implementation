...
- **The Go signing face for this profile is app-side** (`internal/httpsig`), pending an SDK home. Python already ships `sign_agent_binding` and TypeScript ships the verifier; the Go side had neither. Byte parity across all three is held by the shared vectors (`testdata/pop-signature-base-vectors.json`, `testdata/pop-sign-vectors.json`).
