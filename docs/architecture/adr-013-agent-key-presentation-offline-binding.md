...
**Status:** Accepted (2026-06-11); **D5 + D6.1 amended (2026-06-11)** — the binding principal is always the **Agent**, and enforcement defaults **ON wherever the edge is technically capable**. The relay-path conformance this requires (agent-signed `ExecuteTransaction`, multisig verification of all hop signatures, default-ON flip) is a follow-up slice. · **Enforcement hardcoded OFF (2026-07-27), then RESTORED (2026-07-29)** — the interim bearer posture is superseded; `RAMP_ENFORCE_BINDING` is a configuration variable again and defaults ON where capable. See the two amendments under D6.1 and **ADR-023**.
...
The proto narrative (`ramp.proto` (module `github.com/RAMP-Protocol/protocol`), "Retrieval-URL identity binding") fixes the *what*: "require the fetcher to present its public key and an RFC 9421 signature over the retrieval request, and check `thumbprint(presented key) == agent_identity_hash`." It does **not** fix the *how* — the wire carrier for the presented key is unspecified. The proto's only inline-JWK reference is the Web Bot Auth directory (`WBAFile.keys`, of type `JsonWebKey`) — a key *announcement*, not the fetch request.
...
2. **On a multi-hop / relayed path (`MCP → Broker → Exchange`) the request is multisig, and the Exchange verifies every signature.** The Broker is a relay: it adds its own relay signature for hop integrity and audit but **preserves the agent's**, and it carries the Exchange reply back to the agent. The Exchange verifies **all** hop signatures, then binds `agent_id` — and records `transaction_log.agent_identity_hash` — to the **agent's** verified key (the `requester` principal), attesting the relay separately (`allow_broker_relay`, ADR-006). On the direct path the agent is the sole signer and this collapses to the obvious case.
   ...
   **The signatures form an ordered forwarding chain, not parallel co-signatures.** The agent emits `sig1` over the RAMP base covered set (`@method`, `@target-uri`, `content-digest`, `authorization`). Each relay appends `sigN` whose covered set is the base set **plus** the RFC 9421 §2.4 dictionary-member component `"signature";key="sigN-1"`, so `sigN` cryptographically commits to its immediate predecessor. The Exchange enforces that the labels are contiguous `sig1..sigN` in header order and that each `sigN` (N>1) covers exactly its predecessor; a relay therefore cannot strip, reorder, or substitute a peer's signature without invalidating the chain. The earlier "co-sign the same body in parallel" reading — where every signature covered the identical component set and order was not bound — is **superseded**: parallel co-signing left the ordered set unprotected, which the chain closes.

3. **The Exchange bounds the chain depth.** It rejects a request carrying more signatures than `WellKnownManifest.max_intermediary_hops + 1` (the published intermediary ceiling plus the originating agent's own signature — default 4 + 1 = 5) with the SDK verify seam's too-many-hops sentinel → Connect `resource_exhausted` (HTTP 429) and an `outcome=hop_budget` audit line, before any signature is cryptographically verified. The bound is an **Exchange-terminal** policy: relays do not apply it to their own ingress (it would double-count the hop they are about to add). This discharges the future obligation the Broker `hopguard` comment anticipated.
...
**Implementation status (interim).** As shipped in the MR that introduces this ADR, the Exchange still binds `agent_id` to the proven *transport* caller, which on the relay path `MCP → Broker → Exchange` is the Broker's relay key — not the agent: the Broker re-originates and re-signs the `ExecuteTransaction` with its relay key (`src/broker/internal/xclient/signing_transport.go`), so the agent's signature is absent on the Exchange hop. On that interim a capable edge would reject legitimate agent fetches, so enforcement ships **OFF by default** until the D5 agent-binding lands. The **direct-agent** path (agent signs `ExecuteTransaction`, kid == requester.id) already satisfies D5 and is exercised with `RAMP_ENFORCE_BINDING=true` in the obligation / PoP e2e.

This **replaces** the prior "unsatisfiable on the relay path / deferred to Web Bot Auth" framing. The relay-path binding is not unsatisfiable: it requires the agent to sign the `ExecuteTransaction`, the Broker to relay it as multisig (preserving the agent's signature), and the Exchange to verify all hop signatures and bind to the agent (D5). That is a scoped change — agent-signed execute + multisig-aware verification + the default-ON flip — not a WBA-era deferral.

#### Amendment (2026-07-27) — enforcement hardcoded OFF, switch removed

Edge enforcement is **hardcoded OFF** and the `RAMP_ENFORCE_BINDING` switch is **removed** from the edge configuration. In the custodial registry flow (ADR-017) the private key that signs the offer acceptance lives in Vault and never reaches the agent, so the fetcher can never prove possession of the key the URL is bound to — any deployment with enforcement on refuses **every** registry-flow fetch with 403. Because a single misconfigured deployment is enough to break all agents of a publisher, the posture is not configurable: an operator-set `RAMP_ENFORCE_BINDING` value is ignored.

Consequences while this amendment stands:

- Delivery URLs are **bearer credentials** (short TTL + TLS) on **all** edges — the D6 CloudFront-native posture becomes the universal one. The "stolen URL is useless without the key" property does not hold.
- The Exchange **still** binds `agent_id` into the signed URL (D5 unchanged), and the edge's proof-of-possession code is kept dormant and covered by tests — a future re-enable only has to restore the wire from config to the check.
- The long-term shape — restoring proof-of-possession in a way the custodial flow can satisfy (e.g. an agent-held throwaway key attested by the registry), or committing to hardened bearer — is decided by the amendment below.

#### Amendment (2026-07-29) — enforcement restored, switch reinstated

The 2026-07-27 amendment above is **superseded**. `RAMP_ENFORCE_BINDING` returns to the edge configuration and defaults **ON wherever the edge is capable** — D6.1 as originally written. D6 is untouched: CloudFront-native still cannot run the check and keeps the bearer posture.

The custodial objection that amendment recorded is answered by moving the *fetch*, not by weakening the *edge*. The premise — "the fetcher can never prove possession of the key the URL is bound to" — held only while the fetcher was the agent. In the registry flow the registry holds the key, so making the registry the fetcher makes the proof producible by construction: it signs the bound GET with the very key it signed the offer acceptance with, which is the key the Exchange derived `agent_id` from. See **ADR-023**.

Consequences of restoring it:

- **The property in D3 holds again, and holds more widely than before.** A stolen URL is useless without the key — including to another registry user, because the registry signs each fetch as the *calling* agent and the edge rejects a `keyid` that does not match the URL's `agent_id`.
- **`RAMP_ENFORCE_BINDING` is a variable, not a second hardcode.** Deployments roll independently, so an operator who must ship a Worker ahead of a registry that cannot yet present the key needs a way not to break every fetch meanwhile. Opting out is explicit and is a downgrade to bearer security.
- **The alternative this ADR itself floated — an agent-held throwaway key attested by the registry — is rejected**, and the reason is this ADR's own premise. The attestation would have to be verified by the edge, and the edge verifies **fully offline, no JWKS fetch** (see Context). An attestation the edge cannot check is not a binding; making it checkable means giving the edge a live trust path back to the registry, which is the offline property traded away. Registry-side fetching keeps the edge's contract exactly as specified and changes only who calls it.
- **The signing face for this profile ships in all three languages**, in the protocol module `github.com/RAMP-Protocol/protocol`. Go was the last to get one — Python already shipped `sign_agent_binding` and TypeScript shipped the verifier the edge runs — so it was built app-side under `internal/httpsig` and has since moved upstream. What stays app-side is the verifying half's signature-base builder, which the SDK does not export because only a verifier needs it. Byte parity across all three is held by the shared vectors (`testdata/pop-signature-base-vectors.json` for the base string, `testdata/pop-sign-vectors.json` for the emitted headers).
...
- Follow-up: agent-signed `ExecuteTransaction` on the relay path, multisig verification of all hop signatures as an ordered forwarding chain, the Exchange hop bound, and the edge enforcement default-ON flip (D5 / D6.1).
...
- `ramp.proto` (module `github.com/RAMP-Protocol/protocol`) — "Retrieval-URL identity binding" narrative; `agent_identity_hash` field; `WBAFile.keys` / `JsonWebKey` (the directory's inline-JWK use, distinct from the fetch — and the protocol's only one).
- The single RFC 7638 thumbprint helper now ships in the protocol SDK: Go via `internal/rampwellknown`, which wraps `github.com/RAMP-Protocol/protocol/sdk/go/helpers`, and TypeScript via `@ramp-protocol/sdk-l1/thumbprint`. Byte-parity against the shared fixtures is guarded by `internal/guards/thumbprint_vectors_test.go` (D4). The Python call site went with the MCP shim, so D4 now covers two languages, not three.
- `@ramp-protocol/sdk-l1/verify` (`verifyEd25519SignedUrl`) and `@ramp-protocol/sdk-l1/pop` (`verifyAgentBinding`) — the URL-signature verifier and proof-of-possession path (D1–D3), consumed by `src/edge/src/app.ts`.
