...
func TestWellKnownAwareResolver_RevokesThroughMiddleware(t *testing.T) {
	...
	resolver, err := wellKnownAwareResolver(ctx, helpers.NewStaticKeyResolver(nil), http.DefaultClient, slog.Default())
	if err != nil {
		t.Fatalf("wellKnownAwareResolver: %v", err)
	}
...
// TestWellKnownAwareResolver_RevokedKeyOverridesStaticListing pins the
// static-vs-revocation PRECEDENCE explicitly: the revoked thumbprint is ALSO
// listed in the static bootstrap file, and the revocation verdict must win —
// the composite resolver halts on the authoritative REVOKED answer before the
// static delegate can re-admit the key. Without this case the precedence is
// only inferred from construction order; a resolver reordering that consulted
// the static file first would keep every bootstrap-listed key alive forever
// after revocation and no other test would notice.
func TestWellKnownAwareResolver_RevokedKeyOverridesStaticListing(t *testing.T) {
	now := time.Now().UTC()
	priv, key := testutil.NewSigningKey(brokerRelaySeed, now.Add(-time.Hour), now.Add(1000*time.Hour))
	tp := testutil.MustThumbprintKey(t, key)
	...
	// The SAME thumbprint the broker will revoke is present in the static
	// bootstrap file — the delegate that must NOT get the last word.
	static := helpers.NewStaticKeyResolver(map[string]ed25519.PublicKey{tp: pub})
	...
	ctx, cancel := context.WithCancel(context.Background())
	t.Cleanup(cancel)
	resolver, err := wellKnownAwareResolver(ctx, static, http.DefaultClient, slog.Default())
	if err != nil {
		t.Fatalf("wellKnownAwareResolver: %v", err)
	}
	srv := newHTTPSigServer(t, resolver)

	// Active + static-listed: verifies.
	if status := signedDiscoverStatus(t, srv, tp, priv); status != http.StatusOK {
		t.Fatalf("active static-listed key: status = %d, want 200", status)
	}

	// Revoke the static-listed thumbprint: the rejection must land despite the
	// static listing (401, no fall-through re-admission).
	origin.SetRevocation(testutil.MarshalRevocation(time.Unix(2000, 0), tp))
	deadline := time.Now().Add(5 * time.Second)
	for {
		if status := signedDiscoverStatus(t, srv, tp, priv); status == http.StatusUnauthorized {
		...
		if time.Now().After(deadline) {
			t.Fatal("revoked static-listed key still verifying after the poll deadline")
		}
		time.Sleep(50 * time.Millisecond)
	}
}
...
// unreachable/malformed, a thumbprint that is ALSO listed in the static bootstrap
// file must NOT keep verifying — we cannot prove it is un-revoked, so we reject.
// The contrast case proves the distinction the fix turns on: a thumbprint
// genuinely absent from a *successfully fetched* directory (ErrKeyUnknown) is
// allowed to fall through to the static file, because the directory is
// authoritative about the keys it lists, not about a key it never carried.
func TestWellKnownAwareResolver_FailsClosedOnFetchFailure(t *testing.T) {
	...
	// The thumbprint IS in the static bootstrap file in both sub-cases — the
	// question is only whether the revocation channel's verdict lets it through.
	static := helpers.NewStaticKeyResolver(map[string]ed25519.PublicKey{tp: pub})

	t.Run("fetch failure fails closed despite static listing", func(t *testing.T) {
		origin := testutil.NewOrigin(nil)
		t.Cleanup(origin.Close)
	...
	t.Run("thumbprint absent from fetched directory falls through to static", func(t *testing.T) {
		origin := testutil.NewOrigin(nil)
		t.Cleanup(origin.Close)
		...
		// must fall through to the static bootstrap file.
...
// TestWellKnownAwareResolver_RevokedStaticKeyRejected pins the fix for the
// static-bootstrap revocation bypass: a thumbprint the broker directory does NOT
// list but that IS present in the broker's revocation list must be rejected —
// even though it is also present in the static bootstrap resolver. Without the
// fix the broker resolver reports ErrKeyUnknown (not a directory key) and the
// composite falls through to the static file, serving a revoked key. The
// contrast case (thumbprint absent AND not revoked → falls through to static) is
// covered by TestWellKnownAwareResolver_FailsClosedOnFetchFailure.
func TestWellKnownAwareResolver_RevokedStaticKeyRejected(t *testing.T) {
	now := time.Now().UTC()
	priv, key := testutil.NewSigningKey("static-only.v1", now.Add(-time.Hour), now.Add(1000*time.Hour))
	tp := testutil.MustThumbprintKey(t, key)
	pub, err := rampwellknown.PublicKey(key)
	if err != nil {
		t.Fatalf("derive pubkey: %v", err)
	}
	// The thumbprint IS in the static bootstrap file — the question is only
	...
	if status := signedDiscoverStatus(t, srv, tp, priv); status != http.StatusUnauthorized {
		t.Fatalf("revoked static-listed key: status = %d, want 401 (revocation must gate the static file)", status)
	}
}
...
// (broker revocation channel ahead of an empty static bootstrap) against
...
func startRevocationResolverServer(t *testing.T, brokerURL string) *httptest.Server {
	t.Helper()
	t.Setenv("EXCHANGE_BROKER_WELLKNOWN_URL", brokerURL)
	t.Setenv("EXCHANGE_REVOCATION_POLL_INTERVAL", "150ms")
	ctx, cancel := context.WithCancel(context.Background())
	t.Cleanup(cancel)
	resolver, err := wellKnownAwareResolver(ctx, helpers.NewStaticKeyResolver(nil), http.DefaultClient, slog.Default())
...
// staticJWKS is a one-key JWKS the bootstrap loader accepts, so a
// buildHTTPSigDeps composition reaches the revocation-authority check instead of
// failing earlier on an empty/missing key file.
const staticJWKS = `{"keys":[{"alg":"EdDSA","crv":"Ed25519","kid":"agent-demo.v1","kty":"OKP","use":"sig","x":"H-puLL4qBiSasuZ-l2iZZS8k53yG-9FBIPVigMLqucc"}]}`

// TestBuildHTTPSigDeps_FailsClosedWithoutRevocationURL pins fail-closed boot: with a valid
// static bootstrap key file but EXCHANGE_BROKER_WELLKNOWN_URL unset, the Exchange
// MUST refuse to boot rather than proceed fail-open with only the static file (a
// revoked key would otherwise keep verifying). The success path (URL set) is
// covered by the other tests in this file that drive wellKnownAwareResolver.
func TestBuildHTTPSigDeps_FailsClosedWithoutRevocationURL(t *testing.T) {
	keysPath := filepath.Join(t.TempDir(), "keys.json")
	if err := os.WriteFile(keysPath, []byte(staticJWKS), 0o600); err != nil {
		t.Fatalf("write keys file: %v", err)
	}
	t.Setenv("RAMP_KEYS_FILE", keysPath)
	...
	_, _, err := buildHTTPSigDeps(context.Background(), slog.Default(), http.DefaultClient)
	if err == nil {
